...
def get_latest_records():
    ...
    for index, row in dfDataRefreshAudit.iterrows():
        ...
        data_update_date_dt = datetime.strptime(data_update_date, '%Y-%m-%d', time(0, 0))
        ...
        dfDataMod_date_dt = datetime.strptime('1970-01-01', '%Y-%m-%d', time(0, 0))
        logging.info(f'Var dfDataMod_date_dt: {dfDataMod_date_dt}')
        if dfDataMod['data_update_date'][0] is not None:
            dfDataMod_date_dt = datetime.strptime(str(dfDataMod['data_update_date'][0]), '%Y-%m-%d', time(0, 0))
