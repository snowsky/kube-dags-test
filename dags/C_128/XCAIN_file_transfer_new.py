...
dag = DAG(
    'XCAIn_Critical_Data_Pipeline_2',
    ...
    try:
        if ENV == 'Dev':
            try:
                ...
                sftp.chdir(f'C-128/C_128_test_delivery/XCAIn_test_3_folder/{oid1}')
            except IOError:
                sftp.chdir(sftp.normalize('.'))
                sftp.mkdir(f'C-128/C_128_test_delivery/XCAIn_test_3_folder/{oid1}')
                logging.info("first oid folder is created")

            try:
                sftp.chdir(f'C-128/C_128_test_delivery/XCAIn_test_3_folder/{oid1}/{oid2}')
            except IOError:
                sftp.chdir(sftp.normalize('.'))
                sftp.mkdir(f'C-128/C_128_test_delivery/XCAIn_test_3_folder/{oid1}/{oid2}')
            ...
            if euid:  # Only attempt to create folder3 if it exists
                try:
                    ...
                    sftp.chdir(sftp.normalize('.'))
                    sftp.mkdir(f'C-128/C_128_test_delivery/XCAIn_test_3_folder/{oid1}/{oid2}/{euid}')
